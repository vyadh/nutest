...
def with-specific-test [] {
    ...
    let results = test-run $"testing --no-color --path ($temp) --match-tests test_foo"
...
def with-test-pattern [] {
    ...
    let results = test-run $"testing --no-color --path ($temp) --match-tests 'test_ba[rz]'"
...
def with-specific-suite [] {
    ...
    let results = test-run $"testing --no-color --path ($temp) --match-suites test_1"
